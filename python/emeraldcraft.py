...
for item in items:
    ...
    d = ""
    if(typ == "Block"):
        d = "./pages/emeraldcraft/blocks/"+englishName+".html"
    else:
        d = "./pages/emeraldcraft/items/"+englishName+".html"
...
for entity in entities:
    ...
    with open("./pages/emeraldcraft/entities/"+englishName+".html", "w", encoding="utf8") as fout:
        fout.write(template.replace("$Page-Name$", name)
                   .replace("$Register-Name$", registerName)
                   .replace("$Page-Image$", largeIcon)
                   .replace("$Page-Description$", ""))
